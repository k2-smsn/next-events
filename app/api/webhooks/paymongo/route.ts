...
export async function POST(req: NextRequest) {
   ...
   if (eventType === 'payment.paid') {
      await supabaseAdmin.rpc('mark_order_paid_by_intent', { p_payment_intent_id: paymentIntentId })
   } 
   else if (eventType === 'payment.failed') {
      await supabaseAdmin.rpc('release_order_hold_by_intent', { p_payment_intent_id: paymentIntentId })
   }
